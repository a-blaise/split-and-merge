...
def anomalies_ndays(subnets, nb_days):
    ...
    #     files[nb_day] = open(path_join(PATH_EVAL, 'anomalies', 'N_DAYS', nb_day, 'test', 'txt'), 'a')
                        ...
                        for nb_day in nb_days:
                            ...
                            if np.abs(mzscore) > T:
                                ...
                                if id_anomaly in tmp_anomalies:
                                    ...
                                    tmp_anomalies[id_anomaly] = 1
                            if date_an == date:
                                print(nb_day, date, feat.attribute, subnet, feat.time_vect[i - nb_day:i], median, mad, mzscore)
...
def accurate_comparison(type_comparison, intervals):
    ...
    for interval in intervals:
        file = open(path_join(PATH_EVAL, 'anomalies', type_comparison, interval,
                              T_ANO, 'txt'), 'r')
...
def additional_infos(subnets, nb_days):
    ...
    for an in anomalies:
        ...
        for i, date in enumerate(DATES):
            if DATES[i] == date_an:
                print('Anomaly', port, date_an)
            for feat in LIST_FEATURES:
                if DATES[i] == date_an and feat.attribute != 'nb_packets':
                    print(feat.attribute)
                for k, v in feat.sub_time_vect.items():
                    if not np.isnan(v[i]):
                        if feat.attribute != 'nb_packets' and DATES[i] == date_an:
                            print(k)
                            if DATES[i] == date_an:
                                for nb_day in nb_days:
                                    ...
                                    print(nb_day, v[i - nb_day:i], round(median, 2), round(mad, 2), round(mzscore, 2))
    ...
    #     tmp_anomalies = {} # key = port | date | nb_day ; value = int (nb occurrences)
    #     for i, date in enumerate(DATES):
    #         packets_date = packets_port[packets.date == int(date)]
    #         if date_an == date:
    #             print('Anomaly', port, date_an)
    #         for feat in LIST_FEATURES:
    #             feat.reset_object()
    #             for subnet in subnets:
    #                 del feat.time_vect[:]
    #                 packets_sub = packets_date[packets_date.key == subnet]
    #                 feat.time_vect.append(packets_sub[feat.attribute].item() if not packets_sub.empty else np.nan)
    #                 if i > len(DATES) - len_period:
    #                     if date_an == date:
    #                         for nb_day in nb_days:
    #                             median = np.nanmedian(feat.time_vect[i - nb_day:i])
    ...
    #                                                 for y in feat.time_vect[i - nb_day:i]])
    #                             mzscore = 0.6745 * (feat.time_vect[i] - median) / mad
    #                             print(nb_day, feat.attribute, subnet, feat.time_vect[i - nb_day:i], median, mad, mzscore)
